...
public class ServiceConfigInterceptorTest {
    ...
    // 构造拦截器
    private final ServiceConfigInterceptor interceptor = new ServiceConfigInterceptor(/* retryEnabled = */ true);

    // service/method
    private final String fullMethodName = MethodDescriptor.generateFullMethodName("service", "method");

    private final MethodDescriptor<Void, Void> methodDescriptor = MethodDescriptor.newBuilder(new NoopMarshaller(), new NoopMarshaller())
    ...
    /**
     * 测试 waitForReady
     * waitForReady 即 等待直到 channel 是 READY 状态
     */
    @Test
    public void withWaitForReady() {
        ...
        // 构建配置
        ManagedChannelServiceConfig parsedServiceConfig = createManagedChannelServiceConfig(serviceConfig);
        // 更新
    ...
    public void handleUpdateNotCalledBeforeInterceptCall() {
        ...
        verify(channel).newCall(eq(methodDescriptor), callOptionsCap.capture());
        assertThat(callOptionsCap.getValue().isWaitForReady()).isFalse();
        assertThat(callOptionsCap.getValue().getOption(RETRY_POLICY_KEY).get()).isEqualTo(RetryPolicy.DEFAULT);
        assertThat(callOptionsCap.getValue().getOption(HEDGING_POLICY_KEY).get()).isEqualTo(HedgingPolicy.DEFAULT);
    }
    ...
    public void withMaxRequestSize_pickSmallerExisting() {
        ...
        interceptor.interceptCall(methodDescriptor, CallOptions.DEFAULT.withMaxOutboundMessageSize(5), channel);
    ...
    public void withMaxRequestSize_pickSmallerNew() {
        ...
        interceptor.interceptCall(methodDescriptor, CallOptions.DEFAULT.withMaxOutboundMessageSize(10), channel);
    ...
    public void withMaxResponseSize_pickSmallerExisting() {
        ...
        interceptor.interceptCall(methodDescriptor, CallOptions.DEFAULT.withMaxInboundMessageSize(10), channel);
    ...
    public void withMaxResponseSize_pickSmallerNew() {
        ...
        interceptor.interceptCall(methodDescriptor, CallOptions.DEFAULT.withMaxInboundMessageSize(5), channel);
    ...
    public void nearerDeadlineKept_existing() {
        ...
        interceptor.interceptCall(methodDescriptor, CallOptions.DEFAULT.withDeadline(existingDeadline), channel);
    ...
    public void handleUpdate_matchNames() {
        ...
        assertThat(interceptor.managedChannelServiceConfig.get().getServiceMethodMap())
                .containsExactly(methodDescriptor.getFullMethodName(), new MethodInfo(methodConfig, false, 1, 1));
    ...
    public void interceptCall_matchNames() {
        ...
        JsonObj name1 = new JsonObj("service", "service");
        JsonObj name2 = new JsonObj("service", "service", "method", "method");
        JsonObj methodConfig0 = new JsonObj("name", new JsonList(name0), "maxRequestMessageBytes", 5d);
        JsonObj methodConfig1 = new JsonObj("name", new JsonList(name1), "maxRequestMessageBytes", 6d);
        JsonObj methodConfig2 = new JsonObj("name", new JsonList(name2), "maxRequestMessageBytes", 7d);
        JsonObj serviceConfig = new JsonObj("methodConfig", new JsonList(methodConfig0, methodConfig1, methodConfig2));
        ManagedChannelServiceConfig parsedServiceConfig = createManagedChannelServiceConfig(serviceConfig);

        interceptor.handleUpdate(parsedServiceConfig);

        String fullMethodName = MethodDescriptor.generateFullMethodName("service", "method");
        MethodDescriptor<Void, Void> methodDescriptor = MethodDescriptor.newBuilder(new NoopMarshaller(), new NoopMarshaller())
                                                                        .setType(MethodType.UNARY)
                                                                        .setFullMethodName(fullMethodName)
                                                                        .build();
        ...
        fullMethodName = MethodDescriptor.generateFullMethodName("service", "method2");
        methodDescriptor = MethodDescriptor.newBuilder(new NoopMarshaller(), new NoopMarshaller())
                                           .setType(MethodType.UNARY)
                                           .setFullMethodName(fullMethodName)
                                           .build();
        interceptor.interceptCall(methodDescriptor, CallOptions.DEFAULT, channel);
        ...
        fullMethodName = MethodDescriptor.generateFullMethodName("service2", "method");
        methodDescriptor = MethodDescriptor.newBuilder(new NoopMarshaller(), new NoopMarshaller())
                                           .setType(MethodType.UNARY)
                                           .setFullMethodName(fullMethodName)
                                           .build();
    ...
    private static ManagedChannelServiceConfig createManagedChannelServiceConfig(JsonObj rawServiceConfig) {
