...
abstract class RetriableStream<ReqT> implements ClientStream {
     ...
     * 当更新状态或者访问 state.buffer，或者创建 substream 属性时必须持有
     */
    ...
    /**
     * 提交流，被提交后的流表示这个请求已经结束，返回给应用层
     *
     * @param winningSubstream 被提交的流
     * @return 提交里流的任务
     */
     ...
     * 创建代表重试或对冲尝试的物理的 ClientStream，返回的 ClientStream 还没有开始
     */
    abstract ClientStream newSubstream(ClientStreamTracer.Factory tracerFactory, Metadata headers);
     ...
     * 判断 Stream 是否READY
     *
     * @return 只要有一个流是 READY 就认为是 READY 了
     */
    @Override
    ...
    public void appendTimeoutInsight(InsightBuilder insight) {
        ...
        // 如果有流已经完成，则为完成的流追加 InsightBuilder
        if (currentState.winningSubstream != null) {
            ...
            // 如果流没有完成，为每一个流追加 InsightBuilder
    ...
    private final class Sublistener implements ClientStreamListener {
         ...
         * 当从远程端点接收到消息时调用
         *
         * @param producer supplier of deframed messages. 用于解帧消息的提供者
         */
        @Override
        public void messagesAvailable(MessageProducer producer) {
            State savedState = state;
            checkState(savedState.winningSubstream != null, "Headers should be received prior to messages.");
    ...
    private static final class State {
         ...
         * 提交 Substream
         *
         * @param winningSubstream 被提交的 Substream
         * @return 状态
         */
        @CheckReturnValue
    ...
    class BufferSizeTracer extends ClientStreamTracer {
         ...
         * 消息被发送到线路，因此，如果不涉及重试或对冲将释放其引用；这里需要长时间持有引用，用于重试，需要增加 substream.bufferNeeded
         */
        @Override
        public void outboundWireSize(long bytes) {
            ...
            synchronized (lock) {
                ...
                if (bufferNeeded > perRpcBufferLimit) {
                    ...
                    long savedChannelBufferUsed = channelBufferUsed.addAndGet(bufferNeeded - perRpcBufferUsed);
